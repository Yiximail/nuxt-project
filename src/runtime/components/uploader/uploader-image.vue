<template>
  <div
    ...
    @drop="containerDrop"
  >
    <transition-group name="v-uploader-image">
      <v-uploader-image-item
      ...
      />
      <v-uploader-image-insert
        v-if="itemList.length < maxlength"
        key="__image-insert"
...
const props = withDefaults(
  defineProps<{
    ...
    /** 组件样式配置 */
    ui?: DeepPartial<UI["uploaderImage"]>
    /** 大小 */
    size?: Size
  }>(),
  {
...
const { ui } = useUi("uploaderImage", props)
...
<style scoped>
.v-uploader-image-move,
.v-uploader-image-enter-active,
.v-uploader-image-leave-active {
  transition:
    opacity 0.3s cubic-bezier(0.4, 0, 0.2, 1),
    transform 0.3s cubic-bezier(0.4, 0, 0.2, 1);
}

.v-uploader-image-enter-from,
.v-uploader-image-leave-to {
  opacity: 0;
